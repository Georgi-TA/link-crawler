...
for anchor in anchors:
	...
	print (page_url)
	print ("----------------")
	formatted_template = formatted_template + template.format(page_url, title, meta_description, meta_keywords, meta_image,
															  og_title, og_description, og_image, og_url, og_site_name, og_type,
															  twitter_card, twitter_site, twitter_creator, twitter_title, twitter_description, twitter_url, twitter_image_src)


filename = urlparse(url).netloc
with open(filename + ".html", 'a') as f:
	main_template = open('main_template.html', 'r').read()
	f.write(main_template.format(formatted_template))
